...
## Display Power Control — ✅ Done

Implemented via `display_power` tool in clawpi-tools using `wlr-randr`. Also added `system_poweroff` for shutting down the Pi. Both gated behind `services.clawpi.powerControl.enable` (default: true) with passwordless sudo for the kiosk user.

Remaining ideas:
- **DDC/CI brightness** — `ddcutil` for HDMI monitor brightness control
- **Schedule support** — systemd timers for automatic screen-off at night
